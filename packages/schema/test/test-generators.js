...
assertContains(basicTS, 'id: number;', 'auto-generated id')
...
assertContains(basicSQL, 'CREATE SEQUENCE users_seq START 1;', 'per-table sequence')
assertContains(basicSQL, "id INTEGER PRIMARY KEY DEFAULT nextval('users_seq')", 'auto PK')
...
assertContains(compSQL, 'organization_id INTEGER REFERENCES organizations(id)', 'optional FK (no NOT NULL)')
assertContains(compSQL, 'user_id INTEGER NOT NULL REFERENCES users(id)', 'required FK')
assertContains(compSQL, 'post_id INTEGER NOT NULL REFERENCES posts(id)', 'post FK')
...
assertContains(optRelSQL, 'category_id INTEGER NOT NULL REFERENCES categories(id)', 'required belongs_to is NOT NULL')
assertContains(optRelSQL, 'author_id INTEGER REFERENCES authors(id)', 'optional belongs_to is nullable')
assertNotContains(optRelSQL, 'author_id INTEGER NOT NULL REFERENCES', 'optional belongs_to has no NOT NULL')
assertContains(optRelTS, 'categoryId: number;', 'required FK field is non-optional in TS')
assertContains(optRelTS, 'authorId?: number;', 'optional FK field is optional in TS')
