...
//
// Usage:
//   bun run bump                 # full release, bump patch (default)
...
if (args.length === 0 || LEVELS.has(args[0])) {
  await fullRelease(args[0] || 'patch');
} else {
  await selectiveRelease(args);
}

// ── Selective release: bump and publish specific packages ──
...
async function selectiveRelease(packageNames) {
  ...
  // Commit and push
  console.log('\nCommitting...');
  const names = bumped.map(b => `${b.name}@${b.new}`).join(', ');
  const msgFile = join(ROOT, '.git', 'COMMIT_MSG');
  writeFileSync(msgFile, `Publish ${names}`);
  run('git add -A');
...
// ── Full release: bump everything, rebuild, test, publish ──

async function fullRelease(level) {
