...
>
> **Remaining:** the bulk of the `compileForCheck` reconciliation (load-bearing until its construct is inlined — top-level declarations, **fat-arrow return-type syntax** (blocked on the `=>` annotation-vs-operator grammar ambiguity), overload/class machinery); deleting the heuristic map + LSP spill (the fault-B insurance, Track B); and schema field symbols (Track C / fault C). Net production line count stays ~flat by design — see *Surface-area collapse*.
