...
> - **Phase 2 (partial)** — the **diagnostic-equivalence gate** (`test/types/diagnostic-corpus.js`, in `test:all`); inline emission of **function return types** (`def` and **thin-arrow assignments**, now self-sufficient — no header copy) and **typed destructuring** on the check path; **fat-arrow assignments** recognized in the shadow interleave so their own `declare function` header is moved (fixing the editor-only **TS2630** collision with the `let` binding); the **param-copy**, **typed-local hoist**, and **arrow-param transfer** reconciliation passes deleted; **TS1064 / TS2393 / TS2394 recovered** (`SKIP_CODES` down 7 → 4 structural-only codes — 2389/2391/2567/2842). Runtime output byte-identical throughout.
>
> - **Phase 2 (fat-arrow return syntax)** — `(x):: R => body` now parses: the type collector stops at a depth-0 `=>` in arrow-return position (symmetric with `->`), so the `=>` is read as the arrow operator, not a TS function-type arrow. A function-TYPE return must be parenthesized as a whole (`(x):: ((a) => B) => body`); the bare form raises a precise error with a caret + hint instead of silently re-associating the body. The lexer's `tagParameters` was also taught to skip a parenthesized return type when locating the real arrow param list. Completes the fat-arrow story (the inline emitter from the previous slice was already ready).
>
> **Remaining:** the bulk of the `compileForCheck` reconciliation (load-bearing until its construct is inlined — top-level declarations, overload/class machinery); deleting the heuristic map + LSP spill (the fault-B insurance, Track B); and schema field symbols (Track C / fault C). Net production line count stays ~flat by design — see *Surface-area collapse*.
...
### Worked example — the `as`-cast case: type→node association loss for postfix forms

> **Added after the fact.** Captures the `expr as Type` cast work (branch `feat/as-cast`, PR #46) as a concrete, recent motivation for the *generic node-metadata* mechanism this RFC should ultimately provide. The cast shipped a **contained, per-feature** workaround (route A, below); it deliberately does *not* generalize, which is the argument *for* RFC-12's generic propagation. File:line refs are against `main` for the stable metadata channels; the cast-specific code lives on `feat/as-cast`/PR #46 (not yet on `main`), so it's cited by file + branch, not line.

**The problem (worked example): type→node association is lost for postfix expression forms.** The cast follows the same shape as `::` annotations — strip the type *pre-parse* and attach it to a carrier token. That only survives if the carrier becomes a **leaf** in the s-expression. Two forms work, three silently don't:

| Source | Carrier token | Becomes a leaf? | Narrows? |
| --- | --- | --- | --- |
| `x as Foo` | `IDENTIFIER` | yes (identifier atom) | ✅ |
| `obj.p as Foo` | `PROPERTY` | yes (member-access leaf) | ✅ |
| `foo() as Foo` | `CALL_END` | **no** — structural closer | ❌ |
| `arr[i] as Foo` | `INDEX_END` | **no** — structural closer | ❌ |
| `(expr) as Foo` | `)` | **no** — structural closer | ❌ |

For the bottom three the carrier is a **structural closing token** that the reduce action *discards* (it exists only to delimit the call/index/paren; nothing copies its metadata onto the resulting node). The type string evaporates. Runtime erasure is fine either way — the cast leaves zero trace at runtime — but the **type-checker narrowing is silently lost** for exactly the forms where a cast is most useful (`(makeBase() as Rich).label`). No error fires; the cast just does nothing.

**Root cause — the two non-generic metadata channels (the core RFC-12 insight).** Rip has exactly two ways to carry sidecar data alongside the s-expression, and *neither* propagates generically through reduce actions:

1. **Token `.data` → leaf `String` wrapper.** The parser's lex adapter copies `token.data` onto a `new String(val)` (`src/compiler.js:5050-5054`, inside the `lex()` shim at `5046-5061`) — but *only* when the token's value becomes a leaf node. The readers `meta()`/`str()` (`src/compiler.js:29-30`) check `node instanceof String`, so they see metadata on a wrapped **string leaf** and **never** on an array node. A structural closer's `.data` is read by nothing.
2. **`.loc`/`subLocs` on array nodes.** Source positions are attached **per reduce action, by hand**, via `__sub(node, subLocs)` / `__spl(child)`, gated by `__subLocsOn` (`src/parser.js:13-24`). Array nodes are freshly allocated *inside each reduce action* by array-literal spread (`[...$[$0-2], $[$0]]`), which copies only **indexed elements**, not arbitrary own/non-enumerable props — so `subLocs` survives one level *only* because it is manually re-threaded in **~400 generated cases**. There is **no generic node-property propagation**: a property set on a child node does not reach the parent unless a reduce action explicitly carries it.

So a postfix cast has nowhere durable to live: the leaf-`String` channel is gone the moment the carrier is structural, and the `subLocs` channel is position-only and hand-threaded. **This is precisely the gap a "unified emitter / exact positions" RFC-12 closes** — it's the same single-cause family as faults A/B/C (metadata that must survive the AST but rides a retrofitted, non-generic side channel).

**The workaround we shipped (route A) — and why it's a per-feature patch, not the general fix.** Rather than rely on a carrier token, route A adds **one grammar production** so the type rides a real node:

- A new in-stream marker token `CAST` carries the type as an **opaque string** — the type rewriter (`src/types.js`, `isCastAs` + the `rewriteTypes` cast branch) collapses the whole `as Type` run into a single `CAST` token whose value *is* the type string; **the grammar never parses type syntax**.
- Grammar: `o 'Expression CAST', '["cast", 1, 2]'` (`src/grammar/grammar.rip`), with precedence `left CAST` slotted **between `SHIFT` and `RELATION`** — TS-matching, so `a + b as T` ⇒ `(a + b) as T`, and chains (`x as A as B`) nest left-associatively into `['cast', ['cast', x, 'A'], 'B']`.
- Because `CAST` is a real token, the reduction fires **after** the full postfix expression is built, so the `cast` node *wraps* the real node — call/index/paren results narrow too. The emitter (`emitCast`/`castWrap`, `src/compiler.js`) **erases at runtime** and re-materializes `(expr as Type)` only on the `inlineTypes` check path. Worked example + all five forms: `test/types/12-cast.rip`; runtime-erasure tests: `test/rip/cast.rip`. (Branch `feat/as-cast`, PR #46.)

This *works* — narrowing survives for every form — but it does **not** generalize. It cost a bespoke token, a grammar production, a precedence slot, and an emitter handler to make type metadata survive parsing for **one** construct. Every future construct that needs type/assertion metadata to outlive the parse (a `satisfies`-style operator, inline type guards, decorator-ish annotations on arbitrary expressions, …) would need its *own* node or hit the same evaporation. The per-feature cost is the argument for the generic mechanism.

**What full RFC-12 should generalize (the ask).** A **generic mechanism to carry type/assertion/position metadata onto s-expr nodes through reduce actions** — e.g. generic sidecar propagation (a node-keyed side table threaded automatically by the action compiler, the same machinery `subLocs` already approximates for positions), or unifying the token-`.data` and `subLocs` channels into one. With it, type info survives for **all** constructs with **no** per-feature grammar node and **no** reliance on the lossy leaf-`String` channel. Concretely it would let a **pure type-rewriter** approach work for casts — the rewriter attaches the type to the carrier and it survives regardless of whether the carrier is a leaf — **retiring the route-A grammar node if desired**, and fixing the *class* of bug rather than this one instance. (The position half is the same generalization the *Leaf-atom positions* note already calls load-bearing; metadata and position are two facets of "a node must be able to carry sidecar data the AST itself doesn't model.")

**Adjacent quirks discovered during this work** (logged so they're not lost; both probed against `3.17.4` on `feat/as-cast`):

- **(a) Reproduces — spurious `::` error from an inline arrow-function type in a *single-line braced* structural type literal.** `y as { f?: (e: Event) => void }` fails with *"Use `:` (not `::`) inside a structural type literal …"* (the param's `:` in `(e: Event)` is misread against the field's `:`). Scope is narrower than first thought: it fires in **`=`-RHS type positions** — the cast `as` RHS **and** a single-line `type H = { f?: (e: Event) => void }` alias — but **not** in a single-colon annotation (`x: { f?: (e: Event) => void } = y` compiles clean) and **not** in a *multiline* type-literal body. Workaround: pull the arrow into a **named type alias** (`type Handler = (e: Event) => void`) — naming the *arrow* type (not the braced literal) avoids it; a multiline body also works.
- **(b) Does NOT reproduce as described.** The claim was `[a, ...arr.map(typedFn)].some (el) ->` infers `el: unknown`. Probed against the real `_ariaPopupDismiss` shape (`[pop, ...els.map(get)].some (el) -> …`, `pop` an intersection augment, `get` returning the `AriaEl` union) and ~7 reductions (disjoint element/map types, inline typed map fn, `any`-returning map fn, the exact pre-fix untyped state): in **every** case `el` infers a **valid** type — `HTMLElement`, the union `B | A`, `A`, or `any` — never `unknown`, and member access on it type-checks. The `el: AriaEl` annotation added in the cast-branch fix is defensive/explicit (it tightened alongside `get`'s return type and `e: Event`), not a rescue from an `unknown` inference. No bug to file here.

**Independent concurrence.** GPT-5.5 was consulted on the direction and agreed the right RFC-12 answer is **generic metadata/position propagation through reduce actions**, *not* a post-parse loc-correlation heuristic that tries to re-associate types with nodes by position after the fact — the same conclusion the *Critical finding* under *Leaf-atom positions* reaches for the source map.
