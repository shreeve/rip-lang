...
UserPublic = User.pick "id", "firstName", "lastName", "email", "phone"
```
...
**This is what the cart does now** (gaps 5 and 6, both fixed): every shape is defined once on the server and `app/types.rip` re-exports them in a single line — `UserPublic` as `User`, plus `OrderSummary`/`OrderDetail` (the order's two wire views, named by representation) and the plain `Product` shape. The build folds the projections to self-contained descriptors and materializes every imported shape into the browser bundle, so nothing server-only ships and the client declares no schema of its own.
...
Verified end-to-end in the cart: `app/types.rip` is a single `export { UserPublic as User, OrderSummary, OrderDetail, Product } from '../api/models.rip'` (define once on the server, import on the client). The shipped `/app` bundle carries a `_shared/api/models.rip` holding just those shapes — no `kind:"model"`, no `CREATE SEQUENCE`, no `@has_many` — and the projection's type flows cross-file (`User` is `Pick<UserData, …>`), so the profile edit form types as `Omit<User, "id">`.
...
A schema value's type is `Schema<Out, In>`, so `keyof typeof UserPublic` resolved to the interface methods (`parse`, `safe`, `pick`, …), not the data fields — making it impossible to derive a field-key type from a schema value; the cart's per-field errors map fell back to a loose `Record<string, string>`. No new machinery was needed — the field-key types the emitter already produces (gap 5 + the model `Data` type) are reachable: a projection's bare type IS its field shape, so `keyof UserPublic` names its field keys directly, and a `:model` exports a fields-only `<Name>Data` type, so `keyof UserData` names a model's columns without its instance methods. Both are exported, so they resolve cross-file from an importing route. The cart's PATCH handler now types its errors map as `Partial<Record<keyof UserPublic, string>>`, so a typo'd key won't compile. (`keyof typeof User`, over the schema *value*, still yields methods — reach for the field-shape type instead.)
...
### 14. An inline `schema :shape` as the `.extend()` argument typed as `unknown` — extend contributed nothing — FIXED ✅

Adding the order-detail projection (`/api/orders/:id` embeds the full product per item) hit this: `OrderDetail = OrderSummary.omit("items").extend (schema :shape; items! OrderDetailItem[])` was correct at runtime, but the derived *type* silently lost every extended field — `order.items` stayed the base `OrderItem[]`, and nothing flagged the drift. `extend`'s signature is `extend<U>(other: Schema<U>): Schema<In & U, In & U>` (`src/schema/dts.js`), but an *anonymous* expression-position schema compiled to a nameless `__schema({...})` call that only matched the `(d: any) => any` fallback overload, so `U` collapsed and `In & unknown` simplified back to `In`. The docs' own algebra example (`AdminUser = User.extend (schema :shape …)`) was this exact pattern.

Fixed by giving anonymous schemas a shadow-only identity: codegen (`src/schema/schema.js`) stamps each expression-position descriptor with a synthesized `__anon: "__AnonSchemaN"` key and records the descriptor on the generator (shadow-TS mode only — runtime descriptors are untouched, so anonymous schemas are never registered and can't collide in `__SchemaRegistry`); the type emitter (`src/schema/dts.js`) then emits a field-type alias plus a `__schema` overload keyed on that marker, so the inline schema resolves to a precise `Schema<T, T>` and `extend` infers `U` correctly. Pinned by `test/types/10-validation.rip` §5 (an `.omit(...).extend (schema :shape …)` chain whose extended field must typecheck).

The gap had a runtime sibling in the bundle path: projection *folding* (gap 6's `foldProjections`) also required extend's argument to be a bare identifier, so a projection extended with an inline schema couldn't fold — and the bundle materializer then refused to ship it ("isn't a shippable schema"), a 500 on every page load. Fixed in `foldParseChain`/`foldApplyOp`: an inline schema node carries its descriptor on itself, so the fold consumes it directly (no name resolution); field-less kinds (`:enum`/`:union`) bail as before. Pinned by `test/schema/extract.test.js` ("folded extend() accepts an inline anonymous schema argument").
