...
| `examples/form/app/index.html`                                                        |      Yes       | `data-src=app data-mount=App data-reload`             | **Would auto-enable routing.** Single-page form mounting `App` directly — needs `data-router="false"` or migration to a route file.           |
...
Net migration cost: **one file** (`examples/form/app/index.html`) needs an explicit opt-out, and the example would arguably benefit from being migrated to file-based routing anyway.

### Trade-offs

- **Default flip for routing**: Today apps opt *in* to file-based routing. After this change, having a `routes/` directory opts you in automatically. The migration audit above shows this is safe — only the form example crosses the threshold, and its routes/ contains a single `index.rip` that could equally well be a file-based route.
