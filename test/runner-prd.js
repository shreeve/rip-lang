...
/**
 * Rip Test Runner (PRD version)
 *
 * Same as runner.js but uses compiler-prd.js (Predictive Recursive Descent parser)
 * This allows testing that both table-driven and PRD parsers produce identical results.
 *
 ...
 *
 * Usage:
 *   bun test/runner-prd.js test/rip                 # Run directory
 *   bun test/runner-prd.js test/rip/operators.rip   # Run file
 *   bun test/runner-prd.js test/rip test/cs2        # Multiple paths
 */

import { readFileSync, readdirSync, statSync } from 'fs';
import { join, extname, relative } from 'path';
import { compile } from '../src/compiler-prd.js';
...
async function main(args) {
  ...
  if (args.length === 0) {
    args = ['test/rip'];
    console.log(`${colors.cyan}Running default: test/rip (PRD parser)${colors.reset}\n`);
  }
  ...
  console.log(`${colors.bright}Running ${testFiles.length} test file(s) with PRD parser...${colors.reset}`);
