...
**Blanket suppression (6 codes in `SKIP_CODES`):** 2389, 2391, 2393, 2394, 2567, 2842. These fire on structural artifacts of the shadow-TS retrofit (a declare-header and a compiled body are two views of one symbol — overload patterns, enum declarations). But the mute is *global*, and these are **not** safe to suppress unconditionally: the same codes also fire on genuine user mistakes, which then vanish. Demonstrated — `bun run test:suppressed` shows a duplicate definition (2393) and an incompatible overload (2394) that raw `tsc` flags but `rip check` reports clean. This is fault A's user-facing leak; recovering it is part of rip-lang RFC 12 (Unified emitter).

**Recovered (RFC 12 phase 2):** **1064** ("return type of async function must be Promise") was in `SKIP_CODES` until the return type was emitted inline on the implementation (`async function f(): T`, [src/compiler.js](../../../src/compiler.js)). It now fires position-mapped on the user's own annotation — a mis-annotated async return (`:: string` instead of `:: Promise<string>`) is reported, while a correct `:: Promise<T>` passes. The enforced [diagnostic-equivalence gate](diagnostic-corpus.js) (`bun run test:diagnostic-corpus`, in `test:all`) asserts this recovery and that the type-clean corpus relies on zero global mutes; the recovery for each remaining code flips automatically the moment it leaves `SKIP_CODES`.
