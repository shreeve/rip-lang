...
**Blanket suppression (4 codes in `SKIP_CODES`):** 2389, 2391, 2567, 2842. These fire *only* on structural artifacts of the shadow-TS retrofit (a declare-header and a compiled body are two views of one symbol — overload patterns, enum declarations); none is reachable by a genuine user mistake in testing, so blanket suppression is correct for them. The user-reachable leaks have all been recovered (below).

**Recovered (RFC 12 phase 2):** **1064, 2393, 2394** — the user-reachable subset — have left `SKIP_CODES`:
- **1064** ("return type of async function must be Promise") fires once the return type is emitted inline on the implementation (`async function f(): T`, [src/compiler.js](../../../src/compiler.js)): a mis-annotated async return (`:: string` instead of `:: Promise<string>`) is now reported, while a correct `:: Promise<T>` passes.
- **2393** ("duplicate function implementation") and **2394** ("overload signature not compatible with implementation") now surface real mistakes — the same function defined twice, or a typed overload the implementation can't satisfy. Their *structural* occurrences (on compiler-injected overload signatures) were already dropped by the position-based `isInjectedOverload` check, so removing the blanket mute surfaced only genuine user errors with no false positives on legitimate overloads.

The enforced [diagnostic-equivalence gate](diagnostic-corpus.js) (`bun run test:diagnostic-corpus`, in `test:all`) asserts each recovery (the mistake now fires) and that the type-clean corpus relies on zero global mutes; the recovery for any remaining code flips automatically the moment it leaves `SKIP_CODES`.
