...
// the clean stream. This sidesteps converting emitProgram byte-identically, and
// reordering is handled for free (a moved fragment carries its marker).
//
...
// swallowed by string post-processing (.slice/.trim) surfaces as a loud error
// rather than a silently wrong map.

const MARK_OPEN = '\uE000'; // start of an open tag
const MARK_SHUT = '\uE001'; // start of a close tag
const MARK_SEP  = '\uE002'; // terminates the id within either tag
...
function stripMarkers(markedCode, recorder) {
  ...
  const readTag = (i) => {
    // i points at the lead sentinel; read digits until SEP. Returns { id, next }.
    let j = i + 1;
...
export { SourceMapGenerator, vlqEncode, vlqDecode, parseSourceMap, buildLineMap, mergeChunks, mergeChunksWithInlineMap, MarkerRecorder, stripMarkers };
